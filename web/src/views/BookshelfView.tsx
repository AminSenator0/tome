...
export const BookshelfView: React.FC<BookshelfViewProps> = ({ onSelectBook, onNavigateToPipeline }) => {
  ...
  const loadBooks = async () => {
    ...
    try {
      ...
      alert(err.message || t('bookshelf.loadFail'))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDelete = async (folder: string, title: string) => {
    if (!window.confirm(t('bookshelf.deleteConfirm', { title }))) return
    try {
      await Api.deleteBook(folder)
      setBooks((prev) => prev.filter((b) => b.folder !== folder))
    } catch (err: any) {
      alert(err.message || t('bookshelf.deleteFail'))
    }
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-10 gap-4">
        <div className="lg:col-span-3 rounded-3xl bg-card p-6 flex flex-col justify-between soft-shadow border-0">
          <div>
            <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
              {t('bookshelf.totalVolume')}
            </div>
            <div className="text-3xl font-semibold text-foreground tracking-tight mt-1">
              {formatNumber(totalWords)}
              <span className="text-sm font-normal text-muted-foreground ml-2">{t('common.words')}</span>
            </div>
          </div>
          <div className="pt-4 flex items-center justify-between text-xs text-muted-foreground">
            <span>{t('bookshelf.activeBooks')}</span>
            <span className="text-foreground font-semibold">{t('bookshelf.volumes', { n: books.length })}</span>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 rounded-3xl bg-card p-6 flex flex-col justify-between soft-shadow border-0">
          <div>
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                {t('bookshelf.translationProgress')}
              </span>
              <Badge variant="success">
                {t('bookshelf.pctComplete', { pct: overallPercent })}
              </Badge>
            </div>
          ...
          <div className="grid grid-cols-3 gap-2 pt-4 text-xs">
            <div>
              <div className="text-[11px] text-muted-foreground">{t('bookshelf.chapters')}</div>
              <div className="text-foreground font-semibold mt-0.5">
                {formatNumber(totalTransChapters)}/{formatNumber(totalChapters)}
              </div>
            </div>
            <div>
              <div className="text-[11px] text-muted-foreground">{t('bookshelf.tokens')}</div>
              <div className="text-foreground font-semibold mt-0.5">
                {totalTokens > 0 ? `${(totalTokens / 1000).toFixed(1)}k` : '0'}
              </div>
            </div>
            <div>
              <div className="text-[11px] text-muted-foreground">{t('bookshelf.estCost')}</div>
              <div className="text-foreground font-semibold mt-0.5">
                {formatNumber(totalCostToman)} {t('common.tomanShort')}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-3 rounded-3xl bg-card p-6 flex flex-col justify-between space-y-4 soft-shadow border-0">
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <span>{t('bookshelf.refreshLibrary')}</span>
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2">
          <div className="flex items-center gap-3">
            <h2 className="text-lg font-semibold tracking-tight text-foreground">{t('bookshelf.catalog')}</h2>
            <Badge variant="secondary">{t('bookshelf.items', { n: filtered.length })}</Badge>
          </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center p-16 text-muted-foreground rounded-3xl bg-card soft-shadow">
            ...
            <span className="text-xs">{t('bookshelf.scanning')}</span>
          </div>
        ) : filtered.length === 0 ? (
          <div className="rounded-3xl p-12 text-center bg-card soft-shadow">
            ...
            <h3 className="text-base font-semibold text-foreground">{t('bookshelf.noBooks')}</h3>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-start justify-between gap-2">
                      <div className="space-y-1">
                        ...
                        <div className="text-xs text-muted-foreground line-clamp-1">
                          {b.authors && b.authors.length > 0 ? b.authors.join(', ') : t('bookshelf.unknownAuthor')}
                        </div>
                      </div>
                    ...
                    <div className="pt-2">
                      <div className="flex items-center justify-between text-xs text-muted-foreground mb-2">
                        <span>{t('bookshelf.progress')}</span>
                        <span>{t('bookshelf.chProgress', { done: b.translated_chapters, total: b.total_chapters, pct })}</span>
                      </div>
                    ...
                    <div className="flex flex-wrap items-center gap-1.5">
                      {b.has_docx && (
                        <a
                          ...
                          title={t('bookshelf.downloadDocx')}
                        >
                          {t('common.docx')}
                        </a>
                      )}
                      ...
                      {b.has_original && (
                        <a
                          ...
                          title={t('bookshelf.downloadOriginal')}
                        >
                          {t('common.original')}
                        </a>
                      )}
                      {b.image_count > 0 && (
                        <a
                          ...
                          title={t('bookshelf.downloadImages')}
                        >
                          {t('common.images')}
                        </a>
                      )}
